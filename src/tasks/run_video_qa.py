...
from src.datasets.dataset_video_qa import (AlproVideoQADataset,
                                           VideoQACollator)
from src.modeling.alpro_models import AlproForSequenceClassification, AlproForOpenEnded
...
def mk_qa_dataloader(task_type, anno_path, map_vid_path,  lmdb_dir, cfg, tokenizer, is_train=True, return_label=True):
    ...
    datalist = []
    idx = 0
    if cfg.task not in ['next_qa', 'next_qa_oe']:
    ...
    if cfg.task not in ["next_qa", "next_qa_oe"]:
        ...
        with open(cfg.ans2label_path, 'rb') as f:
            loaded_label = pickle.load(f)

        # pick top 4000 vocab
        if cfg.task == "next_qa":
            for i in range(4000):
                label2ans[i] = loaded_label.idx2word[i]
                ans2label[label2ans[i]] = i
        else:
            label2ans = loaded_label.idx2word
            ans2label = loaded_label.word2idx
    ...
    dataset = AlproVideoQADataset(
        task_type=cfg.task,         # 任务类型，也就是 next_qa 这个任务
        datalist=group_datalist,    # datalist
        tokenizer=tokenizer,
        img_lmdb_dir=lmdb_dir,      # visual 数据集路径
        vocab=loaded_label,         #
        ans2label=ans2label,        #
        label2ans=label2ans,
...
def setup_model(cfg, is_train=True, device=None):
    ...
    add_attr_list = [
        "num_labels", "classifier", "cls_hidden_scale", "loss_type", "task", "train_batch_size", "ans2label_path"
    ]
    ...
    # load weights of BLIP BertLMHead
...
def start_training(cfg):
    ...
    for step, batch in enumerate(InfiniteIterator(train_loader)):
        ...
        num_frm = cfg.num_frm
        # (B, T=num_clips*num_frm, C, H, W) --> (B, num_clips, num_frm, C, H, W)
        # (2, 16, 3, 224, 224) ---> (B, 1, 16, 3, 224, 224)
        new_visual_shape = (bsz, num_clips, num_frm) + batch["visual_inputs"].shape[2:]
        visual_inputs = batch["visual_inputs"].view(*new_visual_shape)
        logits = []
        # print("======visual_inputs:", batch["visual_inputs"].shape, "==========")
        # print(new_visual_shape)
        for clip_idx in range(num_clips):
            # (B, num_frm, C, H, W)
            mini_batch["visual_inputs"] = visual_inputs[:, clip_idx]
            # mini_batch["n_examples_list"] = batch["n_examples_list"]
