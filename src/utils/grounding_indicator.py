# grounding indicator: 用于将视频分解为 casual scene 和 compleme`nt scene
...
def padding_mask_k(seq_q, seq_k):
    """ To mask invaild k(all dim are 0), and assign -inf in softmax, seq_k of shape (batch, k_len, k_feat) and seq_q (batch, q_len, q_feat).
        q and k are padded with 0. pad_mask is (batch, q_len, k_len).
    In batch 0:
    [[x x x 0]     [[0 0 0 1]
     [x x x 0]->    [0 0 0 1]
...
class AttentionScore(nn.Module):
    ...
    def forward(self, q, k,  scale=None, attn_mask=None, softmax_mask=None):
        ...
        # linear projection
        # MLP
        q = self.linear_q(q)    # [3, 1, 1024]
        k = self.linear_k(k)    # [3, 16, 1024]

        # 1 / 32
        scale = q.size(-1)**-0.5

        # MLP(v_l) · MLP(q_g)^T
        attention = torch.bmm(q, k.transpose(-2, -1))   # [3, 1, 16]
...
class Grounding_Indicator(nn.Module):
    ...
    def forward(self, q_global, v_local, attn_mask=None):
        ...
        # 计算每个 clip 的注意力分数
        # 再用一个 softmax 计算概率 p
        fg_score = self.fg_att(q_global, v_local, attn_mask=attn_mask)  # [bs, 1, 16]
        bg_score = self.bg_att(q_global, v_local, attn_mask=attn_mask)
        # print("bg_score.shape", bg_score.shape)     # (10, 40, 197)
        #
        # print("q_global.shape", q_global.shape)     # (10, 40, 768)
        # print("v_local.shape", v_local.shape)      # (10, 197, 768)

        # 连接 fg_score 和 bg_score
