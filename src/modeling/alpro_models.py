...
from src.modeling.med import (BertEmbeddings, BertEncoder,
                                BertLMPredictionHead, BertModel, BertPooler, BertLMHeadModel,
                                BertPreTrainedModel)
...
# from src.utils.grounding_indicator import Grounding_Indicator
...
class AlproForPretrain(AlproBaseModel):
    ...
    def forward(self, batch):
        ...
        vtm_loss, vtm_logits, vtm_labels, encoder_outputs_pos = self.compute_vtm(text_embeds=text_embeds,
                                                                                 ...
                                                                                 sim_v2t=sim_v2t.clone(),
                                                                                 # for hard mining
                                                                                 sim_t2v=sim_t2v.clone(),
                                                                                 # for hard mining
...
class AlproForSequenceClassification(AlproBaseModel):
    def __init__(self, config, video_enc_cfg, input_format='RGB'):
        ...
        # self.indicator = Grounding_Indicator(hidden_size=768, tau=100, is_hard=True, dropout_p=0.5)
    ...
    def forward(self, batch):
        ...
        text_output = self.text_encoder(batch['text_input_ids'], attention_mask=text_input_mask, return_dict=True, mode='text')
        text_embeds = text_output.last_hidden_state

        # forward visual
        # timeSformer asks for (b, c, t, h, w) as input.
        # (B, 16, 3, 224, 224) -> (B, 3, 16, 224, 224)
        visual_inputs = visual_inputs.transpose(1, 2)
        ...
        prediction = prediction.view(-1, self.num_labels)     # in next_qa, num_labels = 5. prediction: (bs, 5)

        if targets is not None:
            # loss = F.cross_entropy(prediction, targets.view(-1))      # targets = batch['labels'].

            loss = F.cross_entropy(prediction, targets)      # targets = batch['labels'].
        else:   # evaluation mode
            loss = 0

        return dict(loss=loss, logits=prediction)
...
class AlproForOpenEnded(AlproBaseModel):
    def __init__(self, config, video_enc_cfg, tokenizer, input_format='RGB'):
        super(AlproForOpenEnded, self).__init__(config, video_enc_cfg=video_enc_cfg)
    ...
    def forward(self, batch, is_train=True):
        ...
        if is_train:
            ...
            answer_output = self.text_decoder(ans_input_ids,                    # answer text 对应的 token
                                              attention_mask=ans_input_mask,
                                              encoder_hidden_states=question_output.last_hidden_state,
                                              labels=answer_targets,            # 将 answer token 的 pad token 换成了 -100
                                              return_dict=True,
                                              reduction='none')
            ...
            outputs = self.text_decoder.generate(input_ids=bos_ids,     # encoder 的第一个 token
                                                 max_length=10,         # 最大长度
                                                 min_length=1,          # 最短长度
                                                 num_beams=num_beams,   # beam search 参数
                                                 eos_token_id=self.tokenizer.sep_token_id,  # text 的 end token
                                                 pad_token_id=self.tokenizer.pad_token_id,  # text 的 pad token
                                                 **model_kwargs)        # encoder 得到的 encoder_hidden_states 及
...
class AnsNavieTrans(nn.Module):
    ...
    def forward(self, hidden, decoder_input):
        """
         decode answer
