...
class AlproPretrainSparseDataset(AlproBaseDataset):
    ...
    """
    def __init__(self, dataset_name, map_vid_path, datalist, tokenizer, img_lmdb_dir, img_db_type, txt_dir,
        ...
        self.map_vid_path = map_vid_path
    ...
    def __getitem__(self, index):
        ...
        for _ in range(num_retries):
            ...
            if self.dataset_name == 'next_qa':
                video_id = str(data_sample.video)  # for next_qa
                text = str(data_sample.statement).strip()

                map_vid = load_json(self.map_vid_path)

                video_path = os.path.join(self.img_db_dir, map_vid[str(video_id)] + self.video_fmt)
                # LOGGER.info(text)
