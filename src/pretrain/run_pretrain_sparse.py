...
def mk_captions_pretrain_dataloader(dataset_name, anno_path, video_dir, txt_dir, cfg, tokenizer, 
    ...
    elif dataset_name == "next_qa":
        datalist = pd.read_csv(txt_dir)
        # print(datalist)
        LOGGER.info('Found {} entries for next_qa'.format(len(datalist)))
    else:
        raise ValueError("Invalid dataset_name")

    if dataset_name in ["webvid2m", "next_qa"]:
...
def start_training():
    ...
    if not cfg.e2e_weights_path and cfg.use_mpm:
        ...
        print("============= create tokenized promopts =================")
    ...
    for step, (task, batch) in enumerate(train_loader):
        ...
        # log_interval 步后，在 tensorboard 上记录
        ...
        # 如果是 debug 且 step > debug_step，跳出训练
